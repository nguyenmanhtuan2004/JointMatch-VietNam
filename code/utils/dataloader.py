...
def get_dataloader(data_path, n_labeled_per_class, bs, load_mode='semi'): ## HÀM NÀY ĐÃ CHỈNH SỬA
    ...
    if load_mode == 'semi':
        # Tiền xử lý tách từ cho tiếng Việt
        train_l_content = word_segment_vi(train_l_df['content'].tolist())
        train_u_content = word_segment_vi(train_u_df['content'].tolist())

        train_dataset_l = SEMIDataset(
            train_l_content, 
            train_l_df['synonym_aug'].tolist(), 
            train_l_df['back_translation'], 
            labels=train_l_df['label'].tolist()
        )
        train_dataset_u = SEMIDataset(
            train_u_content, 
            train_u_df['synonym_aug'].tolist(), 
            train_u_df['back_translation'], 
            labels=train_u_df['label'].tolist()
        )
        ...
        train_l_content = word_segment_vi(train_l_df['content'].tolist())
        train_dataset_l = SEMINoAugDataset(train_l_content, train_l_df['label'].tolist())
        train_loader_u = None

    train_loader_l = DataLoader(dataset=train_dataset_l, batch_size=bs, shuffle=True, collate_fn=MyCollator(tokenizer))

    dev_content = word_segment_vi(dev_df['content'].tolist())
    test_content = word_segment_vi(test_df['content'].tolist())
    dev_dataset = SEMINoAugDataset(dev_content, labels=dev_df['label'].tolist())
    test_dataset = SEMINoAugDataset(test_content, labels=test_df['label'].tolist())
