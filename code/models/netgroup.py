...
class NetGroup(nn.Module):
    ...
    def init_net(self, net_arch):
        if net_arch == 'vinai/phobert-base':
            net = AutoModelForSequenceClassification.from_pretrained('vinai/phobert-base', num_labels=self.n_classes)
        elif net_arch == 'vinai/phobert-base-2':
            net = TextClassifier(num_labels=self.n_classes)  # TextClassifier phải sửa lại dùng PhoBERT
        net.to(self.device)
        return net
